...
func NewRedLock(key string, confs []*SingleNodeConf, opts ...RedLockOption) (*RedLock, error) {
	...
	for _, conf := range confs {
		newClient := client.NewClient(conf.Network, conf.Address, conf.Password, conf.Opts...)
		r.locks = append(r.locks, NewRedisLock(key, newClient, WithExpireSeconds(r.expireDuration)))
	}
